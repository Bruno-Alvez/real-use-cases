...
def upsert_customers(records: list[dict], supabase_client) -> dict:
    if not records:
...
def get_sync_cursor(supabase_client, table: str, cursor_field: str) -> str:
    result = supabase_client.table(table).select(cursor_field).order(cursor_field, desc=True).limit(1).execute()
...
def ensure_customer_exists(customer_id: int, supabase_client, wc_client) -> bool:
    existing = supabase_client.table("dim_clientes").select("id_cliente").eq("id_cliente", customer_id).execute()
    ...
    supabase_client.table("dim_clientes").upsert(transform_customer(customer), on_conflict="id_cliente").execute()
...

{
    ...
    "level": "INFO",
    "lambda": "silva-nutrition-sync-vendas",
