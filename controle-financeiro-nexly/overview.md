...
Nexly Group built Controle Financeiro to fill that gap: a multi-tenant SaaS platform where companies manage their financial operations end-to-end, with tenant isolation enforced at the database layer so no company's data is ever reachable by another, regardless of what the application code does.
...
The platform delivers four capabilities, all built on a FastAPI backend with a strict router-to-service-to-repository layering, a Next.js dashboard, and PostgreSQL via Supabase with Row Level Security as the tenant isolation boundary.
...
| HTTP client | httpx (Supabase Storage operations) |
