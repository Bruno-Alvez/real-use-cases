...
Monorepo structure with backend API (Fastify) and frontend web application (React). Backend exposes RESTful APIs and WebSocket connections for real-time updates. PostgreSQL database managed through Prisma ORM. Dual WhatsApp integration via WhatsApp client library with isolated sessions for consultants and AI agent.

## Backend Architecture

### Application Server

Fastify application with modular route registration. Core services initialized at startup: Prisma connection, WebSocket server, WhatsApp client sessions (consultant and AI agent), and health check monitoring.
...
- **WhatsAppService**: Manages WhatsApp connection for consultants, handles message routing, maintains chat index in memory
- **AIWhatsAppService**: Isolated WhatsApp session for AI agent, processes incoming messages and triggers AIAgent
- **AIAgent**: Core AI agent logic, conversation state management, LLM integration via LLM API
...
- **Conversation**: AI agent conversation state with stage tracking
...
Two independent WhatsApp client instances with separate session directories:

1. **Consultant Session**: `whatsapp_sessions/consultant/` - Handles human consultant conversations
2. **AI Agent Session**: `whatsapp_sessions/ai-agent/` - Handles automated AI agent conversations
...
AIAgent processes messages through a state machine with stages: INTRO → QUALIFICATION → SIMULATION → CONVERSION → TRANSFERRED. Each stage has specific prompts and validation rules.
...
3. AI classification via LLM API API (with fallback to rule-based classification)
...
### WhatsApp Message Flow (AI Agent)

1. AIWhatsAppService receives message event
2. Routes to AIAgent.processMessage()
3. Agent retrieves or creates conversation record
4. Message saved to database
5. LLM generates response via LLM API API
6. Response sent via WhatsApp client
...
Backend deployed on AWS container runtime with Docker, persistent disk for WhatsApp sessions. Frontend deployed on AWS frontend hosting with environment variables for API endpoints. Database hosted on PostgreSQL with automated backups. Session persistence ensures WhatsApp connections survive deployments.
