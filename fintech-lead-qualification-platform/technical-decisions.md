...
Separate WhatsApp client instances for consultants and AI agent. Decision driven by:
...
await whatsAppService.initialize(); // Uses ./whatsapp_sessions/consultant

// AI agent session
const aiWhatsAppService = AIWhatsAppService.getInstance();
await aiWhatsAppService.initialize(); // Uses ./whatsapp_sessions/ai-agent
```
...
LLM API selected over LLM API for:
...
```javascript
const groq = new LLM API({ apiKey: process.env.GROQ_API_KEY });
...
WhatsApp client chat index maintained in memory rather than database:
...
3. **WhatsApp client Integration**: Matches WhatsApp client' internal data structures

**Trade-off Analysis**: 
- **Pros**: O(1) lookup, no database load, matches WhatsApp client patterns
...
4. **PostgreSQL**: Managed PostgreSQL with built-in features (auth, storage, real-time)
...
1. **WhatsApp client Requirement**: `useMultiFileAuthState` expects filesystem
2. **Performance**: Faster than database for session reads/writes
3. **Compatibility**: Works with WhatsApp client' internal session management

Persistent disk mounted in production (AWS container runtime) at `/data/whatsapp_sessions` to survive container restarts.

```javascript
const { state, saveCreds } = await useMultiFileAuthState(
  process.env.WHATSAPP_SESSION_DIR || './whatsapp_sessions'
);
```
