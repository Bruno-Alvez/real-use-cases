...
def refresh_bling_token_safely(secrets_client, dynamo_client):
    ...
    if lock.acquire():
        try:
            ...
            secrets_client.update_secret(SecretId="nexly/silva-nutrition/bling", SecretString=json.dumps(new_token))
            return new_token
        finally:
            lock.release()
    else:
        ...
        time.sleep(5)
        return secrets_client.get_secret_value(SecretId="nexly/silva-nutrition/bling")
```
...
```hcl
# projects/silva-nutrition/main.tf
...
module "sync_clientes" {
  ...
  secret_arns      = [data.aws_secretsmanager_secret.supabase.arn, data.aws_secretsmanager_secret.woocommerce.arn]
