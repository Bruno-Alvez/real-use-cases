...
### Repositories
Data access layer. Abstracts PostgreSQL client calls, maps database records to domain objects, and handles query construction.
...
- `backend/src/config/postgres.ts` - PostgreSQL client initialization

### Database (PostgreSQL via PostgreSQL)
...
Evidence:
- `database/postgres/postgres_schema_vouchers_campaigns_final.sql` - Schema, triggers, functions
- `database/postgres/postgres_schema_collaborators.sql` - User and permission tables

### Frontend Application
React SPA that communicates with backend API and directly with PostgreSQL for some operations. Manages UI state and user interactions.

Evidence:
- `frontend/src/App.tsx` - Main application component
- `frontend/src/services/postgres/campaigns.ts` - Frontend campaign service
...
Evidence:
- `database/postgres/postgres_cron_daily_updates.sql` - Daily maintenance script
...
Evidence:
- `database/postgres/postgres_schema_collaborators.sql` lines 383-413 - RLS policies
...
Evidence:
- `database/postgres/postgres_cron_daily_updates.sql` - Cron job implementation
...
Evidence:
- `database/postgres/postgres_schema_vouchers_campaigns_final.sql` lines 26-230 - Table definitions
- `database/postgres/postgres_schema_collaborators.sql` lines 15-91 - User tables
