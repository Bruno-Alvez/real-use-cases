...
No explicit timeouts configured for PostgreSQL client requests. Relies on default Node.js HTTP timeout (typically 2 minutes) and PostgreSQL client defaults.

**Evidence:**
- `backend/src/config/postgres.ts` - No timeout configuration
- PostgreSQL client uses default fetch timeout
...
**Evidence:**
- `database/postgres/postgres_schema_vouchers_campaigns_final.sql` lines 28, 82, 185, 223 - UNIQUE constraints
...
4. Verify PostgreSQL connection (`DATABASE_URL`, `DATABASE_SERVICE_KEY` env vars)
5. Check rate limit status (429 errors indicate limit exceeded)

**Where to look:**
- Application logs: Console output (Winston)
- Database logs: PostgreSQL dashboard → Logs
...
**Evidence:**
- `database/postgres/postgres_cron_daily_updates.sql` - Cron job script
- `database/postgres/postgres_schema_collaborators.sql` lines 78-91 - audit_logs table
