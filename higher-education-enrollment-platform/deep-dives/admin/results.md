...
- `database/postgres/postgres_schema_vouchers_campaigns_final.sql` lines 304-340 - Automatic status updates
...
- `database/postgres/postgres_cron_daily_updates.sql` lines 29-54, 70-94 - Cron job logging
...
4. **Database query latency** - Add timing around PostgreSQL client calls in repositories
...
11. **Database connection pool usage** - Monitor PostgreSQL connection pool (if exposed)
...
// Add: const startTime = Date.now() before PostgreSQL call
...
- `database/postgres/postgres_cron_daily_updates.sql` - Where to add cron metrics
