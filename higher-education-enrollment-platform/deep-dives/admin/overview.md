...
The Higher Education Enrollment Platform manages student enrollment workflows for a higher education institution. It handles candidate registration, document management, voucher and promotional campaign administration, and enrollment tracking through a Kanban-style interface. The system enforces business rules around discount eligibility, campaign limits, and voucher validity periods.

## Integrations

- **PostgreSQL**: Primary database for all entities (students, enrollments, vouchers, campaigns, collaborators, documents)
- **JWT-based authentication**: User authentication and session management (via frontend direct connection)
- **private object storage**: Document file storage (referenced in frontend services)
- **PostgreSQL Functions**: Database-level business logic for discount calculations, validation, and status updates

Evidence:
- `backend/src/config/postgres.ts` - PostgreSQL client configuration
- `frontend/src/lib/postgres.ts` - Frontend PostgreSQL client
- `database/postgres/postgres_schema_vouchers_campaigns_final.sql` - Database functions and triggers
...
- `database/postgres/postgres_cron_daily_updates.sql`
