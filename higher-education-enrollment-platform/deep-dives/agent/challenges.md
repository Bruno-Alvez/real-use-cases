...
- **messaging provider (message sending)**: 20 seconds (`AbortSignal.timeout(20000)`)
- **ViaCEP (address lookup)**: 10 seconds (`AbortSignal.timeout(10000)`)
- **LLM API (LLM calls)**: 30 seconds (configured in LLM API client)
...
- **messaging provider**: No retry (timeout or final error)
- **LLM API**: No explicit retry (BullMQ retries entire job if it fails)
- **PostgreSQL**: No explicit retry (PostgreSQL client may have internal retry)
...
**Consequence**: If webhook receives the same message twice (e.g., messaging provider retry), two jobs will be created and processed.
...
**Evidence**: `src/index.ts`, `src/tools/postgres.ts`, `src/tools/protocol.ts`
...
**Consequence**: If messaging provider sends many messages simultaneously or external APIs have rate limits, system may fail without backpressure control.

**Considerations**:
- messaging provider controls rate of messages received on webhook
- External API rate limits (LLM API, ViaCEP) are not handled in code
...
**Evidence**: `deployment manifest` (if exists), `README.md`

### CI/CD Pipeline

No CI/CD pipeline configured (GitHub Actions, GitLab CI, etc.). Deploy is manual via application logs.
...
1. Check if messaging provider is sending webhooks (provider logs)
2. Check webhook URL configured correctly
3. Check if server is accessible (ngrok in dev, AWS container runtime in prod)
...
1. Check messaging provider credentials (env vars: `ZAPI_TOKEN`, `ZAPI_INSTANCE_ID`)
2. Check messaging provider error logs (`src/tools/zapi.ts`)
3. Check timeout (20s may be insufficient if API is slow)
4. Check messaging provider rate limits

#### LLM API fails

1. Check `OPENAI_API_KEY` is configured
2. Check LLM API rate limits (LLM API dashboard)
...
- **Production**: AWS container runtime logs dashboard or stdout/stderr of container
