...
Read operations use PostgreSQL anon key. Write operations (create student, create enrollment) use service role key.

**Evidence**: `src/tools/postgres.ts`, `src/services/postgres.ts`
...
**Evidence**: `src/config/constants.ts`, `src/tools/postgres.ts`
...
- **PostgreSQL Integration**: `src/tools/postgres.ts`, `src/services/postgres.ts`
- **ViaCEP Integration**: `src/tools/viacep.ts`
- **LLM API Integration**: `src/services/openai.ts`, `src/agents/intentClassifier.ts`
