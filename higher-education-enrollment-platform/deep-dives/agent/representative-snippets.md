...
export async function sendWhatsAppMessage(
  ...
  try {
    ...
    logger.info(
      ...
      'WhatsApp message sent via messaging provider'
    );
  } catch (error) {
    ...
    logger.error(
      ...
      'messaging provider network error'
    );

    throw new ZApiError('Failed to send WhatsApp message via messaging provider', phone, error);
  }
}
```

**Context:** messaging provider client with 20s timeout, configurable headers, structured error handling, test mode.
...
export async function createEnrollment(enrollmentData: {
  ...
  const postgres = getPostgreSQLAdmin();
...
**Context:** Write operation in PostgreSQL. No explicit transaction (PostgreSQL doesn't easily support multi-table transactions). Student and enrollment creation are separate operations.
