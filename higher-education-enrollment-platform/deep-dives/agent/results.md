...
- **API calls**: message sending, ViaCEP lookup, LLM API call
...
- **Errors per API**: count logs with `error` per service (messaging provider, ViaCEP, LLM API, PostgreSQL)
...
**Metric**: Error rate per service (messaging provider, ViaCEP, LLM API, PostgreSQL)

**How to measure**: Count specific errors per service in logs

**Where to instrument**: `src/tools/zapi.ts`, `src/tools/viacep.ts`, `src/services/openai.ts`, `src/tools/postgres.ts` - increment counters in catch blocks
...
**Where to instrument**: `src/tools/postgres.ts` - increment counter after creating enrollment
...
**Where to instrument**: `src/agents/orchestrator.ts` - save timestamp on first message, `src/tools/postgres.ts` - calculate difference when creating enrollment
...
**Metric**: Percentage of timeouts per service (messaging provider, ViaCEP, LLM API)
