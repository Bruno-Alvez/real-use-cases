...
- Use serverless functions (AWS Lambda, AWS frontend hosting)
...
- Fallback: if LLM API fails, falls back to state machine

**Consequences**:
- Cost: each message generates LLM API API call (variable cost per use)
- Latency: adds external API call latency (LLM API) per message
- Less predictable: LLM may misinterpret (mitigated with prompts and validation)
- External dependency: system doesn't work if LLM API is down (fallback to state machine)

**Future improvements**:
- Cache LLM API responses for similar messages
...
**Why chosen**:
- Rate limits: external APIs (messaging provider, LLM API, ViaCEP) have rate limits
...
- Cost: fewer LLM API/PostgreSQL calls in case of persistent failure
...
## 10. Timeout 20s on messaging provider, 10s on ViaCEP, 30s on LLM API

**Decision**: Different timeouts per service: messaging provider 20s, ViaCEP 10s, LLM API 30s.
...
**Why chosen**:
- messaging provider 20s: message sending may take time (network, processing)
- ViaCEP 10s: public API, may be slow, but response is simple
- LLM API 30s: LLM may take time to generate response, especially with large context
- Balance: not too aggressive (avoids false negatives), not too conservative (avoids long wait)

**Consequences**:
- If API is slow but responds in 25s, messaging provider fails but LLM API doesn't (different behavior)
