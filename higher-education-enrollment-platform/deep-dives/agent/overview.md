...
- **messaging provider**: Receives WhatsApp message webhooks and sends messages (legacy support for Evolution API in code, not used in production)
- **PostgreSQL**: Shared database with web systems (tables: students, enrollments, courses, entry_modalities)
- **ViaCEP**: Address lookup by postal code (fallback: user provides address manually if it fails)
- **LLM API (GPT-3.5-turbo)**: Intent classification and full conversational mode
...
- Student and enrollment creation in PostgreSQL
...
- Integration with shared database (PostgreSQL)
