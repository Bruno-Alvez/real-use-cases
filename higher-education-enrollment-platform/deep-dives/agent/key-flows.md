...
**Trigger**: Webhook POST `/webhook/whatsapp` receives message from messaging provider
...
4. Sends prompt to LLM API with context (history + data + the institution knowledge)
5. LLM responds with natural message and data extraction instructions
6. If data complete: creates student and enrollment in PostgreSQL
7. Generates protocol and password, sends credentials via WhatsApp
8. Saves messages to history (Redis LIST)

**Failure points**:
- LLM API timeout (30s) or API error
- ViaCEP unavailable (fallback: user provides address manually)
- PostgreSQL fails to create student/enrollment
- messaging provider fails to send message

**Handling**:
- LLM API: 30s timeout, fallback to state machine mode if it fails
- ViaCEP: retry 2x with progressive backoff, if it fails asks for manual address
- PostgreSQL: error is logged, user receives friendly error message
- messaging provider: 20s timeout, error is logged but not retried (user can try again)
...
4. Searches candidate in PostgreSQL by CPF
...
- PostgreSQL: implicit client timeout, error is logged and state is reset
- User receives message asking to try again

**Evidence**: `src/tools/validation.ts`, `src/tools/postgres.ts`, `src/agents/enrollmentFlow.ts`
...
**Steps**:
1. Generates protocol: searches last protocol of the year in PostgreSQL, increments (format: `FAM2025XXXX`)
...
4. Creates student in PostgreSQL (`students` table)
5. Creates enrollment in PostgreSQL (`enrollments` table)
6. Sends WhatsApp message with protocol and password
7. Updates state to `ENROLLMENT_CREATED`

**Failure points**:
- Protocol generation fails (PostgreSQL query)
...
**Evidence**: `src/tools/protocol.ts`, `src/tools/postgres.ts`, `src/tools/zapi.ts`
