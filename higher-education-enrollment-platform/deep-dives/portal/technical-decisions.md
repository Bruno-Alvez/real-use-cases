...
## Decision 1: PostgreSQL over Direct PostgreSQL Connection

**Decision:** Use PostgreSQL JS client instead of direct PostgreSQL connection (pg library).
...
**Why Chosen:**
- PostgreSQL provides built-in authentication, storage, and real-time capabilities
...
- PostgreSQL client timeout behavior not customizable
...
**Evidence:**
- `backend/src/config/database.ts` - PostgreSQL client initialization (lines 12-22)
...
## Decision 3: Synchronous payment gateway API Calls (No Retry/Backoff)

**Decision:** Make direct HTTP `fetch()` calls to payment gateway API without retry logic or exponential backoff.
...
- payment gateway API generally reliable (low failure rate assumed)
- No additional infrastructure (queues, workers) needed
- Faster implementation

**Consequences:**
- Transient network failures cause checkout failures (user must retry)
- No automatic recovery from temporary payment gateway outages
- Rate limiting from payment gateway not handled gracefully
...
- Implement circuit breaker to fail fast during payment gateway outages
...
## Decision 7: Name Truncation for payment gateway (30 Character Limit)

**Decision:** Truncate customer names to 30 characters when sending to payment gateway API.
...
**Why Chosen:**
- payment gateway API enforces 30-character limit (causes errors if exceeded)
...
- Potential confusion if truncated name appears in payment gateway receipts

**Future Improvements:**
- Validate name length on enrollment form, show warning
- Implement smart truncation (truncate at word boundary)
- Store full name in database, truncated name only for payment gateway
...
## Decision 8: Payment Values in Reais (Not Cents) for payment gateway Checkout

**Decision:** Send payment amounts in reais (decimal format) to payment gateway checkout API, not cents.
...
**Why Chosen:**
- payment gateway checkout API expects decimal values in reais
- Avoids "value exceeds limit" errors (150,000 reais limit, not 15,000,000 cents)
- Matches payment gateway API documentation for checkout sessions
...
- payment gateway checkout UI still shows installment option (though limited to 1)
...
**Decision:** Do not implement idempotency keys when creating payment gateway checkout sessions.
...
- payment gateway may handle duplicates on their side (not verified)

**Consequences:**
- Duplicate checkout sessions possible if user retries or network retries
- Multiple charges possible (though payment gateway may prevent this)
...
- Use idempotency key in payment gateway request if API supports it
