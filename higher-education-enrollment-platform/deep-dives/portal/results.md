...
**Payment Integration:**
- Seamless checkout experience via payment gateway (PIX and credit card)
...
- PostgreSQL eliminates database management overhead
- Cloud hosting reduces infrastructure maintenance
...
**Database Metrics (via PostgreSQL Dashboard):**
- Query performance (if PostgreSQL dashboard provides)
- Table row counts
- Storage usage
...
2. **payment gateway API Error Rate**
   - Definition: Percentage of payment gateway API calls that fail
   - Calculation: `(failed_asaas_calls / total_asaas_calls) * 100`
   - Where to instrument: All `fetch()` calls to payment gateway API
   - Target: < 1%

3. **Checkout Creation Latency (p95)**
   - Definition: 95th percentile time to create checkout session
   - Calculation: Measure time from request to payment gateway response
...
4. **Database Query Latency (p95)**
   ...
   - Calculation: Measure PostgreSQL query execution time
   - Where to instrument: Repository layer, wrap PostgreSQL calls
   - Target: < 500ms
...
12. **Database Connection Pool Usage**
    ...
    - Where to instrument: PostgreSQL dashboard or connection pool metrics
    - Target: < 80%
...
- Wrap payment gateway API calls with timing and error tracking
...
const asaasApiErrors = new Counter({
  ...
  help: 'Total payment gateway API errors',
  labelNames: ['endpoint', 'status_code']
});
```

**Database Metrics:**
- Wrap PostgreSQL queries with timing
