...
7. Validates student address fields (required by payment gateway)
...
9. Truncates customer name to 30 characters (payment gateway limit)
10. Builds payment gateway checkout payload with customer data, items, callbacks
11. HTTP POST to `https://www.asaas.com/api/v3/checkouts`
12. payment gateway returns checkout session (id, link)
...
- payment gateway API error (rate limit, invalid data, network failure)
- Database save failure after payment gateway success
...
- payment gateway errors: error response parsed, message extracted, returned to frontend
- Database save errors: logged, but checkout session already created in payment gateway (inconsistency risk)
- Voucher errors: silently ignored, checkout proceeds without discount
- No retry on payment gateway API failures
...
- `backend/src/config/payment.ts` - payment gateway configuration (lines 11-19)
...
- Query timeouts: PostgreSQL client timeout, error propagated
