...
- `checkoutService.ts` - payment gateway checkout session creation and management
- `pricingService.ts` - Dynamic pricing calculation based on dates, campaigns, pre-enrollment windows
- `paymentService.ts` - Legacy payment creation (direct payment gateway charges)
...
### Repository Layer
Data access abstraction over PostgreSQL:
...
### Database
PostgreSQL via PostgreSQL with:
...
- `backend/src/config/database.ts` - PostgreSQL client configuration

### External Services
- **payment gateway API**: Payment gateway integration
  - Base URL: `https://www.asaas.com/api/v3`
  - Endpoints: `/checkouts`, `/customers`, `/payments`
  - Authentication: API key in `access_token` header

**Evidence:**
- `backend/src/config/payment.ts` - payment gateway configuration and types
- `backend/src/services/checkoutService.ts` - Lines 326-330, 683-687, 865-869

## Data Boundaries

No multi-tenancy. Single institution (the institution). Data isolation by:
...
- Checkout session creation is synchronous HTTP call to payment gateway
...
- `documents` - Document metadata (file URLs point to private object storage)
...
- `checkout_sessions` - payment gateway checkout session metadata

**Evidence:**
- `database/schema.sql` - Complete table definitions (lines 11-180)

### File Storage
Documents stored in private object storage. Database stores metadata only (file_url, file_name, mime_type).
...
| payment gateway Integration | `backend/src/config/payment.ts`, `backend/src/services/checkoutService.ts` |
