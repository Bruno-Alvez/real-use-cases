...
- No explicit timeout configured for `fetch()` calls to payment gateway API
- Relies on Node.js default timeout (no timeout by default, may hang indefinitely)
- PostgreSQL client uses default timeout (not configured)

**Evidence:**
- `backend/src/services/checkoutService.ts` - `fetch()` calls without timeout option (lines 326, 683, 865)

**Database Query Timeouts:**
- PostgreSQL client default timeout (not explicitly configured)
- No query-level timeout settings

**Evidence:**
- `backend/src/config/database.ts` - PostgreSQL client initialization without timeout config
...
- payment gateway API calls (checkout creation, customer creation)
- Database queries (PostgreSQL client may retry internally, not verified)
...
- Checkout session ID: payment gateway-generated, unique per session
...
- payment gateway may return 429 (Too Many Requests), not handled gracefully
...
- PostgreSQL client doesn't log queries by default
...
- cloud runtime blueprint: `backend/deployment manifest`
- Docker multi-stage build: `backend/Dockerfile`
- Frontend: AWS frontend hosting deployment (configured via `frontend/frontend deployment config`)

**Evidence:**
- `backend/deployment manifest` - AWS container runtime deployment config
- `backend/Dockerfile` - Multi-stage Docker build (lines 1-89)
- `frontend/frontend deployment config` - AWS frontend hosting config
...
- No automated deployment on merge to main (AWS container runtime auto-deploy enabled)
...
**Steps:**
1. Check application logs for error message (application logs or logs)
2. Verify payment gateway API key is configured (`ASAAS_PRODUCTION_API_KEY` env var)
3. Check payment gateway API status (external service)
4. Verify student data (address fields required by payment gateway):
...
**Where to Look:**
- Application logs: application logs → Service → Logs
- Database: PostgreSQL dashboard → Table Editor
- payment gateway dashboard: Check for failed API calls
...
**Symptoms:** Payment completed in payment gateway but enrollment status not updated.

**Steps:**
1. Check `payments` table for enrollment:
   - Query by `enrollment_id`
   - Check `status` field (should be 'paid' after payment)
2. Verify payment reference matches payment gateway charge ID:
   - Query payment gateway API: `GET /payments/{chargeId}`
...
**Where to Look:**
- `payments` table: PostgreSQL dashboard
- payment gateway dashboard: Payment status
...
**Where to Look:**
- `documents` table: PostgreSQL dashboard
- `entry_modalities` table: Required documents configuration
- PostgreSQL logs: application logs (if accessible)
...
**Use Cases:**
- AWS container runtime health check (configured in `deployment manifest`)
- Load balancer health checks
- Monitoring service pings

**Evidence:**
- `backend/src/routes/index.ts` - Health check route (lines 16-22)
- `backend/deployment manifest` - Health check path (line 16)
