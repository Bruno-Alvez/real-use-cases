...
**Trigger**: messaging provider webhook POST `/webhook/whatsapp` receives message.
...
4. Sends prompt to LLM API with context, LLM produces a response and structured fields under a constrained schema; the state machine drives control flow
5. If data complete: validates CPF format and checksum
6. Queries ViaCEP for address (retry 2x with backoff: 1s, 2s)
7. Creates student and enrollment in PostgreSQL
8. Generates protocol and password
9. Sends credentials via WhatsApp
10. Saves message to history

**Failure Points**: Invalid payload (400), Redis unavailable (enqueue fails), LLM API timeout (30s) or API error, ViaCEP unavailable (fallback to manual address), PostgreSQL write failure, messaging provider send failure.

**Handling**: Zod validation returns structured error. BullMQ retries failed jobs (2 attempts, 5s backoff). LLM API timeout falls back to state machine mode. ViaCEP retry with progressive backoff, manual address if fails. PostgreSQL errors logged, user receives friendly message. messaging provider 20s timeout, error logged, no retry.
...
7. HTTP POST to payment gateway API creates checkout session
...
**Failure Points**: Pricing calculation failure, missing address fields, payment gateway API error, database save failure after payment gateway success, voucher validation failure.

**Handling**: Pricing errors return 400 with message. Missing address fields: explicit error listing fields. payment gateway errors: response parsed, message returned. Database save errors: logged, inconsistency risk. Voucher errors: silently ignored, checkout proceeds. No retry on payment gateway failures. No idempotency key.
...
2. Reviews uploaded documents in private object storage
...
**Evidence**: `deep-dives/admin/key-flows.md`, `backend/src/services/VoucherService.ts`, `backend/src/services/CampaignService.ts`, `database/postgres/postgres_schema_collaborators.sql`
...
**Steps**:
1. After enrollment creation in PostgreSQL, portal service calls sync function (Planned: not implemented)
...
6. On failure, error logged, enrollment continues in PostgreSQL system
...
**Handling**: Failures logged for manual reconciliation. No retry mechanism implemented. Enrollment proceeds in PostgreSQL regardless of sync status. Manual reconciliation required for failed syncs.
