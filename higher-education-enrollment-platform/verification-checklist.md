...
- [ ] Shared PostgreSQL database schema documented (`database/schema.sql`, `database/postgres/postgres_schema_vouchers_campaigns_final.sql`)
- [ ] RLS policies defined for admin access control (`database/postgres/postgres_schema_collaborators.sql`)
...
- [ ] payment gateway API integration implemented (`backend/src/services/checkoutService.ts`)
- [ ] messaging provider webhook endpoint implemented (`src/index.ts`)
- [ ] LLM API integration for conversational agent (`src/services/openai.ts`)
- [ ] ViaCEP integration with retry (`src/tools/viacep.ts`)
- [ ] Legacy SQL Server sync mentioned (Planned: not implemented)

## Reliability

- [ ] Timeouts configured for external API calls (Agent: messaging provider 20s, ViaCEP 10s, LLM API 30s)
...
- [ ] Portal: No retry logic for payment gateway API calls (Known gap)
...
- **Admin**: `deep-dives/admin/architecture.md`, `backend/src/services/VoucherService.ts`, `backend/src/services/CampaignService.ts`, `database/postgres/postgres_schema_collaborators.sql`
...
- **Agent**: `deep-dives/agent/architecture.md`, `src/agents/conversationalAgent.ts`, `src/queue.ts`, `src/services/openai.ts`, `src/tools/postgres.ts`
