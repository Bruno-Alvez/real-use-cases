...
async def verify_token(
    ...
    """
    Verify JWT token from PostgreSQL
    """
    import logging
    ...
    try:
        ...
        # Extract user_id from token (PostgreSQL uses 'sub' field)
...
async def upload_avatar_from_bytes(self, user_id: str, filename: str, content: bytes) -> str:
    ...
    # Use PostgreSQL REST API directly to ensure proper content-type handling
    postgres_url = settings.DATABASE_URL
    service_key = settings.DATABASE_SERVICE_ROLE_KEY
    
    upload_url = f"{postgres_url}/storage/v1/object/{AVATARS_BUCKET}/{path}"
    ...
    with httpx.Client(timeout=30.0) as client:
        ...
        if response.status_code not in [200, 201]:
            ...
            logger.error(f"private object storage API error: {response.status_code} - {error_msg}")
