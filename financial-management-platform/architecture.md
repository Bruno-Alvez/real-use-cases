...
- `backend/app/services/storage_service.py` - File upload wrapper for private object storage
- `backend/app/services/kpi_service.py` - Dashboard aggregations

### Repository Layer

Data access abstraction. All queries filter by `company_id` for multi-tenant isolation. Uses PostgreSQL Python client.
...
PostgreSQL via PostgreSQL with Row Level Security (RLS) enabled on all tenant tables.
...
- Upload via PostgreSQL Python client or direct REST API
...
- User management (handled by PostgreSQL, not backend)
...
**Object Storage:**
- private object storage buckets for file uploads
