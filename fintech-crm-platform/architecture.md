...
Fastify application with modular route registration. Core services initialized at startup: Prisma connection, WebSocket server, Baileys sessions (consultant and AI agent), and health check monitoring.
...
- **AIBaileysService**: Isolated WhatsApp session for AI agent, processes incoming messages and triggers AIAgent
- **AIAgent**: Core AI agent logic, conversation state management, LLM integration via Groq
...
2. **AI Agent Session**: `baileys_sessions/ai-agent/` - Handles automated AI agent conversations
...
1. AIBaileysService receives message event
